...
@click.option("--record-macro", help="Record a macro with given name")
@click.pass_context
def submit(ctx, target, url, options, package, custom, owner, timeout,
           ...
           baseline, remote, shuffle, pattern, max, unique, record_macro):
    ...
    try:
        l = submit_tasks(
            ...
            pattern, max, unique, url, baseline, shuffle, record_macro
        )
