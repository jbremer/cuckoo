...
from cuckoo.web.bin.utils import api_get
from django.http import HttpResponse


def wow(request):
    from cuckoo.web.controllers.analysis.analysis import AnalysisController
    results = AnalysisController.get_report(74)
    results_analysis = results["analysis"]

    analysis_path = "/home/dsc/.cuckoo/storage/analyses/74"
    reports_path = os.path.join(analysis_path, "reports")

    r = ReportHTML(analysis_path=analysis_path)
    v = r.generate(results=results_analysis)
...
class ReportHTML:
    ...
    def generate(self, results, interactive=True):
        """Writes report.
        @param results: Cuckoo results dict.
        @param interactive: Static (to be used for pdf) or interactive (browser) .html reports
        ...
        try:
            return self.generate_jinja2_template(
                results=results,
    ...
    @staticmethod
    def table_filter(value, table_type):
        if table_type == "signatures":
            e = ""
        e = ""
    ...
    def generate_jinja2_template(self, **kwargs):
        jinja2_env = Environment(
            ...
            lstrip_blocks=True,
            trim_blocks=True)
        jinja2_env.filters["table_row"] = self.table_filter
    ...
    def write(self, reports_path, template):
        try:
